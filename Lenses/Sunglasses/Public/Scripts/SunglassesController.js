...
function configureLensCustomization() {
    ...
    if (script.properties.api.lensMeshes) {
        if (script.customReflection) {
            ...
            for (var j = 0; j < script.properties.api.lensMeshes.length; j++) {
                if (script.properties.api.lensMeshes[j]) {
                    script.properties.api.lensMeshes[j].mainMaterial = script.properties.api.lensMaterial;
                }
            }
    ...
    if (script.twoHeads) {
        for (var i = 1; i < 2; i++) {
            ...
            if (script.properties.api.faceRetouch) {
                ...
                secondHeadRetouch.getComponent("Component.RetouchVisual").faceIndex = i;
            } else {
